...
class GMVAE:
  ...
  def train_epoch(self, optimizer, data_loader):
    ...
    ari = 100.0 * self.metrics.ari(predicted_labels, true_labels)

    return total_loss, recon_loss, gauss_loss, cat_loss, accuracy, nmi, ari
  ...
  def train(self, train_loader, val_loader):
    ...
    train_history_ari, val_history_ari = [], []

    for epoch in range(1, self.num_epochs + 1):
      train_loss, train_rec, train_gauss, train_cat, train_acc, train_nmi, train_ari = self.train_epoch(optimizer, train_loader)
      val_loss, val_rec, val_gauss, val_cat, val_acc, val_nmi, val_ari = self.test(val_loader, True)
      ...
      if self.verbose == 1:
        ...
        print("Accuracy=Train: %.5lf; Val: %.5lf   NMI=Train: %.5lf; Val: %.5lf Train_ARI: %.3lf; Val_ARI: %.3lf   Total Loss=Train: %.5lf; Val: %.5lf" % \
              (train_acc, val_acc, train_nmi, val_nmi, train_ari, val_ari, train_loss, val_loss))
      else:
        print('(Epoch %d / %d) Train_Loss: %.3lf; Val_Loss: %.3lf   Train_ACC: %.3lf; Val_ACC: %.3lf   Train_NMI: %.3lf; Val_NMI: %.3lf Train_ARI: %.3lf; Val_ARI: %.3lf' % \
              (epoch, self.num_epochs, train_loss, val_loss, train_acc, val_acc, train_nmi, val_nmi, train_ari, val_ari))
      ...
      train_history_ari.append(train_ari)
      val_history_ari.append(val_ari)
    return {'train_history_nmi' : train_history_nmi, 'val_history_nmi': val_history_nmi,
            'train_history_ari': train_history_ari, 'val_history_ari': val_history_ari,
